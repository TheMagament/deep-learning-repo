...
optimizer_params = dict(lr=learning_rate, betas=(0.9, 0.999), eps=1e-08, weight_decay=amount_WeightDecay)


class heyush(nn.Module):
    def __init__(self):
        super(heyush, self).__init__()
        self.CNN1 = nn.Conv2d(1,100, kernel_size=3, stride=2)
        self.CNN2 = nn.Conv2d(100, 200, kernel_size=3, stride=2)
        self.CNN3 = nn.Conv2d(200, 400, kernel_size=3, stride=2)

    def forward(self, input):
        x = self.CNN3(self.CNN2(self.CNN1(input)))
        return x
...
if (evaluation_mode == 1):
    ...
    for epoch in range(num_epochs):
        for i, (images, labels) in enumerate(train_loader):
            ...
            correct = sum(winner_class.data.cpu().numpy() == labels)
            total = labels.size(0)
        ...
        print("Epoch=" + str(epoch) + ", Batch=" + str(i) + ",Accuracy=" + str(correct / total) + ": loss is: %.3f" %
              loss.data[0] + "]")
...
for images, labels in test_loader:
    ...
    correct = correct + sum(winner_class.data.cpu().numpy() == labels)
